# Azure basic sku public IP address deployment
...
Param(
    ...
    [Parameter(Mandatory = $false,
    HelpMessage= "Public IP Address Allocation Method 'Static or Dynamic'")]
    [Alias ('e')]
...
$TemplateFile = "..\modules\public_ip_basic\azuredeploy.json"
...
{ 
    if (    !([string]::IsNullOrEmpty($subscriptionId)) -and `
            ...
            !([string]::IsNullOrEmpty($publicIpName)) -and `
            !([string]::IsNullOrEmpty($publicIpAllocationMethod))  )
    {
    ...
    {
        Write-Host -Object "Either subscription ID, resource group name, deployment name, public IP address name or and public IP address allocation method prefix is missing " -BackgroundColor Red -ForegroundColor White
    }   
}
else
{
